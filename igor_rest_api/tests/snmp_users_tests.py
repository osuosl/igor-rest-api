...
class UsersTestCase(IgorApiTestCase):
    ...
    def create_test_user(self):
        user = Snmpuser(self.test_user, self.test_pass)
    ...
    def test_list_users(self):
        ...
        response = self.client.get(url_for('snmpusers'), headers=self.headers)
    ...
    def test_add_user(self):
        expected_new_user_response = {u'username': self.test_user,
                                      u'location': url_for('snmpuser',
                                                    username=self.test_user,
                                                    _external=True),
                                      u'pdus': url_for('user_pdus',
                                                    username=self.test_user,
                                                    _external=True)}
        ...
        response = self.client.post(url_for('snmpusers'),
                                    data=data,
        ...
        user = Snmpuser.query.filter_by(username=self.test_user).first()
    ...
    def test_user_info(self):
        ...
        response = self.client.get(url_for('snmpuser', username=self.test_user),
                                   headers=self.headers)
        self.assert_200(response)
        print response.json
    ...
    def test_nonexistent_user_info(self):
        response = self.client.get(url_for('snmpuser', username='something'),
                                   headers=self.headers)
        self.assert_404(response)

    def test_remove_root_user(self):
        self.create_test_user()
        response = self.client.delete(url_for('snmpuser',
                                              username=ROOT_USER),
                                      headers=[('Authorization', 'Basic '
                                        + base64.b64encode(self.test_user +
                                                           ':' +
    ...
    def test_remove_nonexistent_user(self):
        response = self.client.delete(url_for('snmpuser', username='something'),
                                      headers=self.headers)
        self.assert_404(response)
    ...
    def test_update_user(self):
        ...
        response = self.client.put(url_for('snmpuser', username=self.test_user),
                                   data=data,
                                   headers=self.headers)
        self.assert_200(response)
